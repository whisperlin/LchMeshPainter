...
namespace LCH
{
    partial class LchMeshPainter
    {
        private  class LchEncode
        {
            ...
            public static byte[] EncodeToTGA( Texture2D _texture2D, int iBytesPerPixel = 4, Compression _compression = Compression.RLE)
            {
                const int iTgaHeaderSize = 18;
                const int iBytesPerPixelARGB32 = 4;
                    ...
                    using (BinaryWriter binaryWriter = new BinaryWriter(memoryStream))
                    {
                        ...
                        else
                        {
                            ...
                            while (iPacketStart < arPixels.Length)
                            {
                                ...
                                RLEPacketType packetType = LchEncode.PacketType(arPixels, iPacketStart);
                                ...
                                {
                                    bool bPreviousEqualsCurrent = LchEncode.Equals(arPixels[iPacketEnd - 1], arPixels[iPacketEnd]);
            ...
            {
                if ((_iPacketPosition != _arData.Length - 1) && LchEncode.Equals(_arData[_iPacketPosition], _arData[_iPacketPosition + 1]))
                {
